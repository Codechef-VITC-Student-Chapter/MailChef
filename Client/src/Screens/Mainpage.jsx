...
function Mainpage() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Header */}
      <header className="bg-blue-200 flex items-center justify-between md:p-8 custom-xss:p-7 relative">
        <div className="flex items-center w-full justify-between">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center">
            <img
              src={iconHeader}
              alt="Header"
              className="md:h-[5.7rem] md:w-[31rem] custom-xss:h-[3rem] custom-xss:w-[20rem] "  // Adjusted width for custom-xss
            />
          </div>

          {/* Exit Icon */}
          <button className=" ml-auto" onClick={() => setIsSpecialView(false)}>
      ...
      <div className="flex flex-1 flex-col md:flex-row md:p-0">
        {!isSpecialView ? (
          <>
            ...
            {/* Sidebar/Menu Icon */}
            <div
              className={`w-full 2xl:w-[6%] md:w-[13%] lg:w-[9%] bg-white flex flex-col items-center py-2  ${
